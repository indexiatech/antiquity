...
public abstract class VersionedGraph<T extends IndexableGraph, V extends Comparable<V>> extends EventIndexableGraph<T>
    ...
    /**
     * A marker property key which indicates that the {@link Element} is
     * removed.
     */
     ...
     *
     * <p>
     * A vertex revision must be available for the specified version, otherwise
     * a null will be returned.
     * </p>
     *
     * @param id The unique id of the vertex
     * @param version The version to get the vertex for
    ...
    public void setPrivateHash(Vertex vertex) {
        ...
        if (vertex instanceof VersionedVertex) {
            ...
            String oldHash = (String) versionedVertex.getBaseVertex().getProperty(VersionedGraph.PRIVATE_HASH_PROP_KEY);
     ...
     * @throws IllegalArgumentException If the specified vertex is not a
     *         {@link VersionedVertex}.
     */
    ...
    public Vertex getVersionConfVertex() {
        ...
        // if (v == null)
        // throw new
        // RuntimeException("Could not find the graph configuration vertex");
    }
    ...
    /**
     * Create a historical vertex which contains the modified vertex content
     * before it was modified.
     *
     * TODO return vertex should be immutable.
     *
     * @param modifiedVertex The modified {@link VersionedVertex}.
     * @param oldValues The old properties values of the modified versioned
     *        vertex.
     * @return {@link Vertex} containing the old data before the vertex was
     *         modified.
     */
    private Vertex createHistoricalVertex(VersionedVertex<V> modifiedVertex, Map<String, Object> oldValues) {
        ...
        // Iterate on the base prop keys as we'r currently working on an active
        // vertex
     ...
     * @param modifiedVertex The modified vertex
     * @param newHistoricalVertex The historical vertex of the modified vertex
     */
    private void addHistoricalVertexInChain(V latestGraphVersion, V newVersion, VersionedVertex<V> modifiedVertex,
        ...
        if (currEdges.iterator().hasNext()) {
            ...
            // TODO: Edge id?
            getBaseGraph().addEdge(null, newHistoricalVertex, currEdge.getVertex(Direction.IN),
                    PREV_VERSION_CHAIN_EDGE_TYPE);

            getBaseGraph()
                    .removeEdge((currEdge instanceof EventEdge) ? ((EventEdge) currEdge).getBaseEdge() : currEdge);
        }

        // TODO: Edge id?
        getBaseGraph().addEdge(null, ((EventVertex) modifiedVertex).getBaseVertex(), newHistoricalVertex,
                PREV_VERSION_CHAIN_EDGE_TYPE);
     ...
     */
    protected Vertex versionModifiedVertex(V latestGraphVersion, V newVersion, VersionedVertex<V> vertex,
    ...
    /**
     * Get the relevant vertex revision from the history for the specified
     * vertex and version. TODO return vertex should be immutable.
     *
     * @param vertex The vertex to find the appropriate version for
     * @param version The version to find the revision for
     * @return A vertex revision for the specified version
     * @throws NotFoundException In case no vertex revision was found for the
     *         specified vertex and version
     */
    public Vertex getVertexForVersion(Vertex vertex, V version) {
        ...
        if (!verRange.contains(version)) {
            ...
            // Iterate over the base (non filtered) edges to seek for the
            // revision chain edge
            ...
            if (!prevVerEdges.iterator().hasNext()) {
                throw ExceptionFactory.notFoundException(String.format(
                        "Cannot find vertex %s in revision history for version [%s].", vertex, version));
            }
    ...
    /**
     * Returns the property value of {@link VersionedVertex} for the specified
     * key. Please note that this method will return the value of the property
     * for the specific version set in the {@link VersionedVertex}.
     *
     * A null will be returned if property doesn't exist for the specified
     * {@link VersionedVertex} in its set version.
     *
     ...
     * @return The value of the specified property key for the specified @{link
     *         {@link VersionedVertex} or null if not found.
     */
     ...
     * @return If the specified element supports Events, then return its base
     *         element.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Raise an exception of the specified {@link Element} does not support
     * events.
     *
    ...
    /**
     * Return the key names of internal properties used by the {@code}
     * VersionedGraph to version the {@link Graph} {@link Element}s.
     *
    ...
    public boolean isHistoricalOrInternal(Element e) {
        if (e instanceof Vertex) {
            return (e.getPropertyKeys().contains(HISTORIC_ELEMENT_PROP_KEY) && ((Boolean) e
                    .getProperty(HISTORIC_ELEMENT_PROP_KEY)));
     ...
     *
     * A versioned vertex contains for sure the 'HISTORIC_ELEMENT_PROP_KEY'
     * property key which defines whether it is historic or not.
     *
    ...
    public boolean isVersionedEdge(Edge edge) {
        return ((edge.getPropertyKeys().contains(HISTORIC_ELEMENT_PROP_KEY) || edge.getLabel().equals(
                PREV_VERSION_CHAIN_EDGE_TYPE)));
    }
     ...
     *
     * @param chain An empty list that will contain the history chain
     * @param v The vertex to get the chain for
     */
